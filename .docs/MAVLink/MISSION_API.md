...
## 3. MAV_FRAME
- "GLOBAL": 기본적으로 WGS84 위도/경도 및 고도가 평균 해수면(Mean Sea Level)에 대해 양수인 전역 좌표 프레임
- "RELATIVE_ALT": 고도는 MSL이 아닌 차량 홈 위치를 기준으로 한다.
- "TERRAIN_ALT": 고도는 MSL이 아닌 지면 수준을 기준으로 한다.
- "LOCAL": 로컬 프레임의 원점은 지구를 기준으로 고정된다. 달리 지정하지 않는 한 이 원점은 차량 위치 추정기("EKF")의 원점이다.
- "BODY": 로컬 프레임의 원점이 차량과 함께 이동한다. (차량 자세에 따른 프레임축의 정렬을 나타내지 않는다.)
### 0: MAV_FRAME_GLOBAL
- 전역(WGS84) 좌표계 + 평균 해수면(MSL)을 기준으로 한 고도 
### 1: MAV_FRAME_LOCAL_NED
- 지구를 기준으로 원점이 고정된 NED 로컬 접선 프레임 (x: 북쪽, y: 동쪽, z: 아래쪽 )
### 2: MAV_FRAME_MISSION
- 좌표계가 아니며 임무 명령을 나타냄, 
- **시뮬레이터에서 프레임 옵션을 이것으로 주고 x=0, y=0 을 주면 제자리에서 takeoff, land 가능** 
### 3: MAV_FRAME_GLOBAL_RELATIVE_ALT
- 전역 좌표계 + 홈 위치를 기준으로 한 고도 
### 4: MAV_FRAME_LOCAL_ENU
- 지구를 기준으로 원점이 고정된 ENU 로컬 접선 프레임 (x: 동쪽, y: 북쪽, z: 위쪽)
### 7: MAV_FRAME_LOCAL_OFFSET_NED
- 차량과 함께 이동하는 원점을 갖는 NED 로컬 접선 프레임 (x: 북쪽, y: 동쪽, z: 아래쪽)
### 10: MAV_FRAME_GLOBAL_TERRAIN_ALT
- AGL 고도(지상고도)를 퐇마한 전역(WGS84) 좌표계
